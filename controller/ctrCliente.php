...
switch ($_GET["op"]) {
    ...
    case "eliminar":
        ...
        $validarCliente = $cliente->mdlSeleccionarRegistros($tabla, $item, $valor);
        if (!empty($validarCliente)) {
            echo json_encode([
                "status" => "error",
                "message" => "No se puede eliminar cliente porque está siendo utilizado en un vehiculo."
            ]);
            return;
        }

        $cliente->mdlEliminarRegistro($valor);
        echo json_encode([
            "status" => "ok",
