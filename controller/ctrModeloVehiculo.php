...
/* TODO: Llamando clases */
    ...
    require_once("../models/mdlModeloVehiculo.php");

/* TODO: Inicializando clases */
    $modelo = new mdlModeloVehiculo();
    ...
    switch($_GET["option"]){
        ...
        case "guardaryeditar":
            if(empty($_POST["token"])){
                ...
                $token = md5($_POST["modelo"] . "+" . $_POST["modelo"]);
                $modelo->mdlRegistro($token, $_POST["marca"]);
            }else{
                $nuevoToken = md5($_POST["modelo"] . "+" . $_POST["modelo"]);
                $modelo->mdlActualizarRegistro($_POST["token"], $nuevoToken, $_POST["marca"]);
            }
            break;
        ...
        case "listar":
            ...
            $item = "token";
            $valor = $_POST["token"];
            $datos=$modelo->mdlSeleccionarRegistros($tabla,$item,$valor);
            $data=Array();
        ...
        case "eliminar":
                $modelo->mdlEliminarRegistro($_POST["token"]);
            break;
    }
